...
var Gpml2Pvjson = function() {
  ...
  this.toPvjson = function(gpmlPathwaySelection, pathwayMetadata, outerCallback) {
    if (_.isString(gpmlPathwaySelection)) {
      var xmlDoc = $.parseXML(gpmlPathwaySelection);
      gpmlPathwaySelection = $(xmlDoc).find('Pathway');
    }

    var xmlns = gpmlPathwaySelection.attr('xmlns');
    gpmlPathwaySelection = instance.fixBiopax(
        instance.addIsPartOfAttribute(instance.makeExplicit(gpmlPathwaySelection)));
    var pvjson = {};
  ...
  this.toPvjsonSource = function(gpmlStringSource, pathwayMetadata) {
    return gpmlStringSource
      .map(function(bufferChunk) {
        ...
        var encodedChunk = he.encode(he.decode(stringChunk), {
          // allowUnsafeSymbols means don't encode "<" and ">"
          // (plus some other symbols we don't care about)
          'allowUnsafeSymbols': true
        });
        return encodedChunk;
      })
      .reduce(function(accumulator, encodedChunk) {
        return accumulator + encodedChunk;
      })
      .map(function(gpmlString) {
        var xmlDoc = $.parseXML(gpmlString);
        return $(xmlDoc).find('Pathway');
      })
      .flatMap(function(gpmlPathwaySelection) {
        return Rx.Observable.fromNodeCallback(instance.toPvjson)(
          gpmlPathwaySelection, pathwayMetadata);
      });
  };
   ...
   program
     ...
     .action(function(gpml) {
       ...
       // haven't figured out how to go from command line to input args
        ...
        Gpml2Pvjson.toPvjson(gpmlPathwaySelection, pathwayMetadata,
