...
var url = require('url');
var StreamGpmlToPvjson = require('./stream-gpml-to-pvjson.js');
var VError = require('verror');

var filename = 'gpml2pvjson-js/lib/index.js';
...
(function() {
  ...
  // I would prefer to use the code from underscore.js or lodash.js, but it doesn't
  ...
  Gpml2013aPvjson10Converter.gpmlToPvjsonSource = function(inputStream) {
    var gpml2pvjsonConverterStream = highland.pipeline(StreamGpmlToPvjson);

    /*
    var newInputStream = highland();
    return RxNode.fromReadableStream(
      RxNode.writeToStream(source, newInputStream, 'utf8')
        .pipe(gpml2pvjsonConverterStream)
    );
    //*/
    ...
    return RxNode.fromReadableStream(
      ...
      .doOnError(
        function(err) {
          var err2 = new VError(err, 'missing file: "%s"', filename);
          throw err2;
        }
      );
    //*/
    ...
    /*
    return RxNode.fromReadableStream(inputStream)
      .doOnError(
        function(err) {
          var err2 = new VError(err, 'missing file: "%s"', filename);
          throw err2;
      ...
      .flatMap(Gpml2013aPvjson10Converter.convertGpmlToPvjson);
    //*/
  };
   ...
   * @param {string} [options.bridgeDbDatasourceName] database name
   * @param {string} [options.identifier] database identifier
   * @param {string} [options.idVersion]
  ...
  Gpml2013aPvjson10Converter.convertGpmlToPvjson = function(input, options, callback) {
    ...
    var bridgeDbDatasourceName = options.bridgeDbDatasourceName;
    var identifier = options.identifier;
    var idVersion = options.idVersion;
    ...
    .each(function(pvjson) {
      var pathwayIri = !!identifier ?
          'http://identifiers.org/' + bridgeDbDatasourceName + '/' + identifier : input;
