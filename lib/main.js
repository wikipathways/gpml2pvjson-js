...
var Gpml2Json = function() {
  var instance = this;
  this.toPvjson = function(gpmlPathwaySelection, pathwayMetadata, callbackOutside) {
    ...
    if (GpmlUtilities.supportedNamespaces.indexOf(xmlns) === -1) {
      callbackOutside('Pathvisiojs does not support the data format provided. ' +
                      'Please convert to GPML and retry.', {});
    } else {
      // test for whether the GPML file version matches the latest version
      // (only the latest version will be supported by pathvisiojs).
      if (GpmlUtilities.supportedNamespaces.indexOf(xmlns) !== 0) {
        ...
        callbackOutside('Pathvisiojs may not fully support the version of GPML provided (xmlns: ' +
              ...
              Anchor: function(callback) {
                ...
                if (anchorsSelection[0].length > 0) {
                  ...
                  anchorsSelection.each(function() {
                    ...
                    pathvisiojs.formatConverter.gpml.anchor.toPvjson(gpmlPathwaySelection,
                        anchorSelection,
                        function(pvjsonElements) {
            ...
            function(err, results) {
              ...
              pvjson.elements.filter(function(element) {
                return (element.type === 'undefined' || element.type === undefined) &&
                    element['gpml:element'] === 'gpml:Interaction';
              ...
              return callbackOutside(null, pvjson);
            });
          }
        ]);
      }
  ...
  this.makeExplicit = function(gpmlPathwaySelection) {
    ...
    if (graphicalElementsSelection.length > 0) {
      ...
      if (nodesSelection.length > 0) {
        ...
        // Below we correct the GPML so that the display in pathvisiojs will
