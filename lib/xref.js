...
Xref.generate = function(args){
  ...
  if (linkoutPattern === 'http://identifiers.org/chebi/$id' && identifier.indexOf('CHEBI') === -1) {
    identifier = 'CHEBI:' + parseFloat(identifier);
  }

  pvjsonXref.id = !!linkoutPattern ? linkoutPattern.replace(/\$id/, identifier) : dataSourceName + ':' + identifier;
  ...
  if (!!organism && !!bridgeDbDbNameCode && !!identifier && identifier.indexOf('WBInteraction') === -1 && (!linkoutPattern || linkoutPattern.indexOf('reactome') === -1)) {
    pvjsonXref['biopax:xref'] = [];
    //pvjsonXref.xref = [];

    // This URL is what BridgeDB currently uses. Note it currently returns TSV.
    var bridgeDbIri = encodeURI('http://webservice.bridgedb.org/' + organism + '/xrefs/' + bridgeDbDbNameCode + '/' + identifier);
    // It would be nice to change the URL to something like the version below. It would also be nice to return JSON-LD.
    //var bridgeDbIri = encodeURI('http://bridgedb.org/' + bridgeDbDbNameCode + '/' + identifier + '/xref');
    /*
    var BridgeDbXref = {
      'id': bridgeDbIri,
      type: 'Xref'
    ...
    //*/
    pvjsonXref['biopax:xref'].push(bridgeDbIri);
    ...
    if (!!linkoutPattern && (linkoutPattern.indexOf('ensembl') > -1 || linkoutPattern.indexOf('ncbigene') > -1)) {
      ...
      //*/
      pvjsonXref['biopax:xref'].push(myGeneIri);
    }
  }
  return pvjsonXref;
};
